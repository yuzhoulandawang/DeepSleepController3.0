...
class DeepSleepService : Service() {
    ...
    private lateinit var powerSaverController: PowerSaverController
    private lateinit var rootDeepSleepController: RootDeepSleepController
    ...
    override fun onCreate() {
        ...
        powerSaverController = PowerSaverController()
        rootDeepSleepController = RootDeepSleepController()
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        return try {
            ...
            when (intent?.action) {
                ACTION_START -> {
                    // 启动深度睡眠
                    Logger.log(Logger.Level.INFO, "DeepSleepService", "启动深度睡眠模式")
                    START_STICKY
                }
                ACTION_STOP -> {
                    // 停止深度睡眠
                    Logger.log(Logger.Level.INFO, "DeepSleepService", "停止深度睡眠模式")
                    stopSelf()
                    START_NOT_STICKY
                }
                else -> START_STICKY
            ...
            Logger.log(Logger.Level.ERROR, "DeepSleepService", "权限异常: ${e.message}")
            e.printStackTrace()
            START_NOT_STICKY
        } catch (e: Exception) {
            Logger.log(Logger.Level.ERROR, "DeepSleepService", "未知异常: ${e.message}")
            e.printStackTrace()
            START_NOT_STICKY
        }
    }
    ...
    private suspend fun enterDeepSleepMode() {
        ...
        if (settings.enablePowerSaverOnSleep) {
            powerSaverController.enablePowerSaver(this)
            Logger.log(Logger.Level.INFO, "深度睡眠", "省电模式已启用")
        }
        
        // 延迟后进入深度睡眠 Hook
        if (settings.deepSleepHookEnabled) {
            delay(settings.deepSleepDelaySeconds.toLong() * 1000)
            rootDeepSleepController.enterDeepSleep()
            Logger.log(Logger.Level.SUCCESS, "深度睡眠", "已进入深度睡眠 Hook 模式")
    ...
    private suspend fun exitDeepSleepMode() {
        ...
        if (settings.deepSleepHookEnabled) {
            ...
            Logger.log(Logger.Level.INFO, "深度睡眠", "已退出深度睡眠 Hook 模式")
        }
        
        // 关闭省电模式（如果配置）
        if (settings.disablePowerSaverOnWake) {
            powerSaverController.disablePowerSaver(this)
            Logger.log(Logger.Level.INFO, "深度睡眠", "省电模式已关闭")
        }
    }
            ...
            while (isActive && rootDeepSleepController.isInDeepSleep()) {
                ...
                if (rootDeepSleepController.isInDeepSleep()) {
                    ...
                    if (settings.deepSleepBlockExit) {
                        ...
                        Logger.log(Logger.Level.DEBUG, "深度睡眠", "维持深度睡眠状态")
                    }
                }
            }
        }
